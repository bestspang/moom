  ...
  return (
    <form onSubmit={handleSubmit(d => mutation.mutate(d))} className="animate-in fade-in-0 duration-200">
      ...
      <Section className="mb-6">
        <h1 className="text-xl font-bold text-foreground mb-4">Upload Transfer Slip</h1>
        <div className="space-y-4">
          <div className="flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-border p-8 bg-muted/50">
            ...
            <p className="text-sm text-muted-foreground">Tap to upload slip image</p>
      ...
      <div className="px-4 pb-8">
        <Button type="submit" className="w-full" disabled={isSubmitting || mutation.isPending}>
          {mutation.isPending ? 'Uploading...' : 'Submit Transfer Slip'}
        </Button>
      </div>
    </form>
  );
