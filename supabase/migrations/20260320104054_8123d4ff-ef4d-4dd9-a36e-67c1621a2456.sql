...
-- Add tier_change to notification_type enum
ALTER TYPE notification_type ADD VALUE IF NOT EXISTS 'tier_change';

-- Re-create evaluate_member_tier with tier change notification
...
BEGIN
  ...
  IF v_current_row IS NOT NULL AND v_current_row.current_tier IS DISTINCT FROM v_best_tier THEN
    DECLARE
    ...
    BEGIN
      SELECT experience_user_id INTO v_notif_user_id
      FROM identity_map
      WHERE admin_entity_id = p_member_id AND entity_type = 'member' AND is_verified = true
      LIMIT 1;

      IF v_notif_user_id IS NOT NULL THEN
        SELECT tier_order INTO v_prev_order FROM status_tier_rules WHERE tier_code = v_current_row.current_tier;
        v_direction := CASE WHEN v_best_order > COALESCE(v_prev_order, 1) THEN 'upgraded' ELSE 'downgraded' END;

        v_notif_title := CASE WHEN v_direction = 'upgraded'
          THEN 'Status Tier Upgraded!'
          ELSE 'Status Tier Changed' END;

        v_notif_message := 'Your status tier has changed from ' || initcap(v_current_row.current_tier) || ' to ' || initcap(v_best_tier) || '.';

        INSERT INTO notifications (user_id, type, title, message, related_entity_type, related_entity_id)
        VALUES (v_notif_user_id, 'tier_change', v_notif_title, v_notif_message, 'member_status_tier', p_member_id);
      END IF;
    END;
  END IF;
