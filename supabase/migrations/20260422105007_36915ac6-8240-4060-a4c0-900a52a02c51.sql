...
-- Fix process_slip_approval: correct enum names
-- Bug: COALESCE(p_package_type, 'class')::transaction_type → wrong enum + wrong value
--   real enum is package_type with values (unlimited|session|pt); default to 'session'
-- Bug: 'completed'::transaction_status → invalid; must be 'paid'
...
BEGIN
  ...
  INSERT INTO transactions (
    ...
    'paid'::transaction_status, p_location_id, p_staff_id, p_note,
