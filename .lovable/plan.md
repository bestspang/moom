...
The edge function checks `event_type === "package_purchase"`.
The SP rules use `action_key = 'package_purchase'`.

**DO NOT change `package_purchase` to `package_purchased` anywhere. That will break the SP pipeline.**
...
| Doc | Status |
|-----|--------|
| `gamification-event-map.md` | ✅ Correct keys + AI safety banner |
